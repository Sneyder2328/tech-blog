...
description: "Setting up Better Auth for secure authentication with email/password and Google OAuth, integrating it with our NestJS backend and React frontend — now with actual vibe coding."
pubDate: 2026-01-23
...
**🤖 Model**: GPT 5.2 Extra High Reasoning (via Cursor)

**📝 Summary**: The AI updated `packages/db/prisma/schema.prisma` with:

- **User model** — Added `emailVerified`, `image` fields and relations to `Session`, `Account` (IDs are UUIDv7)
- **Session model** — New model with `token`, `expiresAt`, `ipAddress`, `userAgent`, `createdAt`, `updatedAt` (timestamps are `timestamptz`)
- **Account model** — New model for OAuth providers and password storage with `providerId`, `accountId`, tokens, `password` field (plus a composite unique constraint)
- **Verification model** — New model for email verification and password reset tokens

**🔧 Adjustments**: None needed. The AI correctly followed my existing conventions.

Here’s the resulting schema (showing the key auth tables):
...
**🎯 Goal**: Add Better Auth to both the API and web packages.
...
**🎯 Goal**: Create the `.env` file for the API with all required secrets.
...
**🔧 Adjustments**: Generated a proper secret with `openssl rand -base64 32` and added my actual Google OAuth credentials.
...
**🤖 Model**: Claude Opus 4.5 Thinking (switched for UI work — it tends to produce cleaner designs than GPT 5.2)

**📝 Summary**: The AI created:
...
- The `BETTER_AUTH_SECRET` was missing — Better Auth silently fails without it
...
**What worked well:**
- Schema updates were nearly perfect — the AI understood my conventions and applied them consistently
...
- Some styling preferences are subjective — I tweaked spacing and colors

**Model choice:**
- **Claude Sonnet 4** for backend/infrastructure work — more precise, follows instructions closely
- **Gemini 2.5 Pro** for UI work — tends to produce more polished designs out of the box
